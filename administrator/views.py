...
class IssueCreateListAPIView(generics.ListCreateAPIView):
    queryset = Issue.objects.all()
    serializer_class = IssueSerializer

class IssueRetrieveUpdateDeleteAPIView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Issue.objects.all()
    serializer_class = IssueSerializer

class TechnicianCreateListAPIView(APIView):
    def post(self,request):
        ...
        password = f"{name}@{random.randint(10000,99999)}"
        user = User.objects.create_user(username=email,email=email,first_name=name,is_active=False,password=password)
        Role.objects.create(user_id = user,role='technician')
        customer = Customer.objects.create(address=address,pincode=pincode,phone=phone,user_id=user)
        TechnicianQualification.objects.create(customer=customer,qualification=qualification,years_of_exp=years_of_exp)
        message = "Dear " + name + ", Your email for logging into the smart appliance services is " + email + " and password is " + password
        from_email = "soumyams999@gmail.com"
        send_mail('Email and Password',f"{name},{email},{password}",'soumyams999@gmail.com',[email])
        return Response({'message':"Technician created and mail send successfully"},status=status.HTTP_201_CREATED)
    
    def get(self,request):
        technicians = TechnicianQualification.objects.all()
        serializer = TechnicianQualificationSerializer(technicians,many=True)
        return Response(serializer.data)

class TechnicianRetrieveUpdateDelete(APIView):
    def get(self,request,pk):
        technicians = TechnicianQualification.objects.get(id=pk)
        serializer = TechnicianQualificationSerializer(technicians)
        return Response(serializer.data,status=status.HTTP_200_OK)
    
    def patch(self,request,pk):
        technician_instance = TechnicianQualification.objects.get(id=pk)
        customer_instance = technician_instance.customer
        user_instance = customer_instance.user_id
        data = request.data
        ...
        user_instance.save()
        customer_instance.address = data.get('address')
        customer_instance.pincode = data.get('pincode')
        customer_instance.phone = data.get('phone')
        customer_instance.save()
        technician_instance.qualification = data.get('qualification')
        technician_instance.years_of_exp = data.get('years_of_exp')
        technician_instance.save()
        return Response({'message':"Technician updated successfully"},status=status.HTTP_200_OK)
    
    def delete(self,request,pk):
        technicians = TechnicianQualification.objects.get(id=pk)
        customer_instance = technicians.customer
        user_instance = customer_instance.user_id
        technicians.delete()
        customer_instance.delete()
        user_instance.delete()
...
class ServiceRejectAPIView(APIView):
    def post(self,request,pk):
        service = Service.objects.get(id=pk)
        service.status = 'rejected'
        service.save()
        return Response({'message':"service rejected"},status=status.HTTP_201_CREATED)
    
class ServiceAcceptAPIView(APIView):
    def post(self,request,pk):
        service = Service.objects.get(id=pk)
        service.status = 'accepted'
        service.save()
        technician = TechnicianQualification.objects.all()
        technician_data = TechnicianQualificationSerializer(technician,many=True).data
        return Response({'message':"service accepted",'technician':technician_data})
    
class ServiceScheduleAPIView(APIView):
    def post(self,request,pk):
        data = request.data
        ...
        service = Service.objects.get(id=pk)
        technician = TechnicianQualification.objects.get(id=technician)
        schedule = Schedule.objects.create(service=service,technician=technician,schedule_date=schedule_date,start_time=start_time,end_time=end_time,service_address=service_address)
        return Response({'message':"service scheduled"},status=status.HTTP_201_CREATED)
